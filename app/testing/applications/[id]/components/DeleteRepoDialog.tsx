...
import { deleteRepositoryAction } from "@/lib/actions/repositories";
...
export function DeleteRepoDialog({
  ...
  async function handleDelete() {
    ...
    const result = await deleteRepositoryAction(repositoryId, applicationId);

    if (result.success) {
      onClose();
    } else {
      setError(result.message ?? "Failed to disconnect repository.");
      setDeleting(false);
    }
  }
    ...
    >
      <div className="relative mx-4 w-full max-w-md rounded-xl border border-border bg-surface p-6 shadow-xl">
        ...
        <p className="mb-6 text-sm text-text-secondary">
          ...
          ? This will not affect the repository itself.
        </p>
