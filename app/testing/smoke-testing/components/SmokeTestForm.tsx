...
interface SmokeTestFormProps {
  ...
  onSubmit: (data: { inputText: string; applicationId: string; repositoryId: string }) => void;
  disabled: boolean;
}
...
export function SmokeTestForm({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!inputText.trim() || inputText.trim().length < 10) {
      newErrors.inputText = "Scenario must be at least 10 characters.";
    }
    if (!selectedAppId) {
    ...
    setErrors({});
    onSubmit({ inputText: inputText.trim(), applicationId: selectedAppId, repositoryId });
  }

  return (
    <form onSubmit={handleSubmit} className="flex h-full flex-col rounded-xl border border-border bg-surface">
      {/* Header — matches Agent Activity header */}
      <div className="flex items-center gap-2 border-b border-border px-4 py-3">
        <div className="flex h-6 w-6 items-center justify-center rounded-md bg-violet-500/10">
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="text-violet-600">
      ...
      <div className="flex-1 space-y-4 p-4">
        {/* Scenario Input */}
        <div>
          <label htmlFor="st-inputText" className="mb-1.5 block text-sm font-medium text-text-primary">
            Test Description
          </label>
          <textarea
            id="st-inputText"
            ...
            className={`h-28 w-full resize-y rounded-lg border bg-surface-inset px-3 py-2 text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-1 disabled:cursor-not-allowed disabled:opacity-60 ${
              errors.inputText
          ...
          />
          {errors.inputText && (
            <p className="mt-1 text-xs text-red-500">{errors.inputText}</p>
          )}
        </div>

        {/* Application Select */}
        <div>
          <label htmlFor="st-applicationId" className="mb-1.5 block text-sm font-medium text-text-primary">
            Application
          </label>
          <select
            id="st-applicationId"
        ...
        {/* Repository Select */}
        <div>
          <label htmlFor="st-repositoryId" className="mb-1.5 block text-sm font-medium text-text-primary">
            Repository
          </label>
          <select
            id="st-repositoryId"
