...
export async function runReviewer(
  ...
  ctx.emit({
    event: "agent_message",
    data: { agent: "reviewer", text: "Reviewing generated script..." },
  });
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (result.passed) {
      ctx.emit({
        ...
        data: {
          agent: "reviewer",
          text: "Script passed all checks. No errors found.",
        },
      });
    ...
    ctx.emit({
      ...
      data: {
        agent: "reviewer",
        text: `Script failed (attempt ${attempt}). Analyzing errors and attempting auto-fix...`,
      },
    });
...
Fix this script and return the corrected version.`,
    ...
    ctx.emit({
      ...
      data: {
        agent: "reviewer",
        text: "Applied fixes. Will re-run to verify.",
      },
    });
  ...
  throw new Error(
    `Script failed review after ${MAX_RETRIES} attempts. Manual intervention required.`
  );
}
